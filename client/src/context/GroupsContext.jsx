...
export const GroupsProvider = ({ children }) => {
  ...
  const addGroup = async (group_name, about_text, cover_picture) => {
    ...
    const response = await fetch('http://localhost:5300/app/groups', {
      method: "POST",
      headers: {
  ...
  const addContactToGroup = async (group_id, contacts_id) => {
    const response = await fetch(`http://localhost:5300/app/groups/${group_id}/contacts/${contacts_id}`, {
        method: "POST",
        headers: {
  ...
  const getGroup = async (group_id) => {
    const response = await fetch(`http://localhost:5300/app/groups/${group_id}`, {
      method: "GET",
      headers: {
          Authorization: `Bearer ${idToken}`
      }
